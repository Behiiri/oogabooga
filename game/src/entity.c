...
// ent[BULLET_ENTITY_MAX]  - ent[MONSTER_ENTITY_MAX]  -> monsters
// ent[MONSTER_ENTITY_MAX] - ent[MAX_ENTITIES]        -> everything else

entity ent[MAX_ENTITIES];
static entity_id max_tile_id = 0;
static entity_id max_bullet_id  = TILE_ENTITY_MAX;
static entity_id max_monster_id = BULLET_ENTITY_MAX;
static entity_id max_entity_id  = MONSTER_ENTITY_MAX;
...
entity_id allocate_monster(void)
{
    int i;
    for (i=BULLET_ENTITY_MAX; i < MONSTER_ENTITY_MAX; ++i)
        if(!ent[i].valid) {
            if(i >= max_monster_id)
                max_monster_id = (entity_id) i+1;
            ent[i].valid = True;
            return (entity_id) i;
        }
    return 0;
...
{
    int i;
    for (i=MONSTER_ENTITY_MAX; i < MAX_ENTITIES; ++i)
        if(!ent[i].valid) {
            if(i >= max_entity_id)
